prob_density <- function(features, mean_values, sd_values, class_val, total_val){
  ...
  for(i in 1:nrow(features)){
    ...
    density_list[i] = sum(row_val)  + log(class_val/(total_val))
  }
  density_list
}
...
for (wi in 1:10)
{
  ...
  #   row_val <- c()
  #   for(j in 1:ncol(ntrbx)){
  #     row_val[j]  <- dnorm(ntrbx[i,j], mean = ptrmean[j], sd = ptrsd[j], log = TRUE)
  #     }
  #   pos[i] = sum(row_val) + log(py1/(py1 + py0)) 
  # }
  ...
  #   row_val <- c()
  #   for(j in 1:ncol(ntrbx)){
  #     row_val[j]  <- dnorm(ntrbx[i,j], mean = ntrmean[j], sd = ntrsd[j], log = TRUE)
  #   }
  #   neg[i] = sum(row_val)  + log(py0/(py1 + py0))
  # }
  ...
  #   row_val <- c()
  #   for(j in 1:ncol(ntebx)){
  #     row_val[j]  <- dnorm(ntebx[i,j], mean = ptrmean[j], sd = ptrsd[j], log = TRUE)
  #   }
  #   pos[i] = sum(row_val) + log(py1/(py1 + py0)) 
  # }
  ...
  #   row_val <- c()
  #   for(j in 1:ncol(ntebx)){
  #     row_val[j]  <- dnorm(ntebx[i,j], mean = ntrmean[j], sd = ntrsd[j], log = TRUE)
  #   }
  #   neg[i] = sum(row_val)  + log(py0/(py1 + py0))
  # }
